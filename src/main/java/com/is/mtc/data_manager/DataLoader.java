	...
	public static void readAndLoad() {
		...
		for (String resourcesZip : resourcesZips) {
			...
			try (ZipFile file = new ZipFile(resourcesZipFile.toString())) {
				List<Tuple> cardZipEntries = new ArrayList<>();
				...
				for (Tuple entry : cardZipEntries) { // Load cards after editions
					if ((boolean) entry.getSecond()) {
						InputStreamReader inputStreamReader = new InputStreamReader(file.getInputStream((ZipEntry) entry.getFirst()));
						BufferedReader br = new BufferedReader(inputStreamReader);
						getSingleCardCdf(br, ((ZipEntry) entry.getFirst()).getName());
						br.close();
						inputStreamReader.close();
					} else {
						getSingleCardJson((JsonObject) parser.parse(loadInputStreamJson(file.getInputStream((ZipEntry) entry.getFirst()))), ((ZipEntry) entry.getFirst()).getName());
					}
				}
